...
"""
# Author: Bill
# Created Time : 2016-08-08 13:33:47
...
class Redis(object):        
    def __init__(self, logpath,  password=None, port=6379, debug=False):
        ...
        self._debug = debug
        self._logger = Log(logpath,level="error",is_console=self._debug,mbs=5,count=3)

    def get_redis_port_list(self,port_list=""):
        # sudo权限，必须授予
        ...
        if port_list:
            ...
            cmdstr = "netstat  -nlpt | grep 'redis-server' | awk '{print $4}'|awk -F: '{print $2}'|uniq"
            disk_space_info = []
...
def main():
    try:
        ...
        if options.is_list == True:
            print redis_ins.get_redis_port_list(port_list)
            return
